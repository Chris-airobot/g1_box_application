...
class SceneConfig:
    ...
    """List of scene files (USD/URDF) to load. Defaults to empty list."""

    rigid_objects: list[RigidObjectConfig] = field(default_factory=list)
    """Standalone rigid objects to instantiate. Defaults to empty list."""
    ...
    def _coerce_none_lists(cls, data: Any) -> Any:
        if isinstance(data, dict):
            if data.get("scene_files") is None:
                data["scene_files"] = []
            if data.get("rigid_objects") is None:
                data["rigid_objects"] = []
        return data
