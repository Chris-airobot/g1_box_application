...
@dataclass(frozen=True)
class DistillConfig:
    """Configuration for DAgger-style teacher-student distillation."""

    enabled: bool = False
    teacher_checkpoint: str | None = None
    teacher_obs_keys: List[str] = field(default_factory=lambda: ["teacher_obs"])
    dagger_only: bool = True
    dagger_anneal: bool = False
    dagger_coefficient: float | None = None
    dagger_coefficient_max: float = 1.0
    dagger_coefficient_min: float = 0.0
    dagger_anneal_start: int = 0
    dagger_anneal_end: int = 2500
