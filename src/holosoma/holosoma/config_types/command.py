...
class MotionConfig:
    ...
    motion_file: str
    """Single motion file (.npz) to track (legacy)."""
    ...
    """Multiple motion files (.npz). If provided, overrides motion_file."""

    motion_dir: str | None = None
    """Directory containing motion files. Used if motion_files is None."""
    ...
    eval_start_timestep: int | None = None
    """For evaluation/play only: force starting at a specific timestep (overrides eval_start_time_s)."""

    eval_start_timestep_range: list[int] | None = None
    """For evaluation/play only: randomize start timestep in [min, max] if provided."""

    eval_start_time_s: float | None = None
    """For evaluation/play only: force starting at a specific time in seconds."""

    eval_start_time_range_s: list[float] | None = None
    """For evaluation/play only: randomize start time in seconds within [min, max]."""

    eval_motion_id: int | None = 0
    """For evaluation/play only: which motion index to use when multiple motions are provided.
    If None or negative, sample a random motion per reset."""
    ...
    """If set, sample a random segment of this duration (seconds) from each motion."""
