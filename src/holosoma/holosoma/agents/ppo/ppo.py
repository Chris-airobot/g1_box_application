...
class PPO(BaseAlgo):
    ...
    def setup(self):
        ...
        self._setup_storage()
        if self.config.distill.enabled:
            self._setup_teacher()
    ...
    def _setup_storage(self):
        ...
        if self.config.distill.enabled:
            self.storage.register("gt_actions", shape=(self.num_act,), dtype=torch.float)

    def _setup_teacher(self):
        distill_cfg = self.config.distill
        if distill_cfg.teacher_checkpoint is None:
            raise ValueError("distill.enabled=True but distill.teacher_checkpoint is None")

        for key in distill_cfg.teacher_obs_keys:
            if key not in self.algo_obs_dim_dict:
                raise ValueError(f"Unknown teacher obs key: {key}")

        teacher_actor_cfg = dataclasses.replace(self.config.module_dict.actor, input_dim=distill_cfg.teacher_obs_keys)
        ...
        checkpoint_path = load_checkpoint(distill_cfg.teacher_checkpoint, self.log_dir)
        checkpoint = torch.load(checkpoint_path, map_location=self.device)
        state_dict = checkpoint.get("actor_model_state_dict") or checkpoint.get("actor_state_dict")
        if state_dict is None:
            raise KeyError(f"Teacher checkpoint missing actor state dict. Keys: {list(checkpoint.keys())}")
        self.teacher_actor.load_state_dict(state_dict)

    def _get_dagger_coefficient(self) -> float:
        cfg = self.config.distill
        if cfg.dagger_coefficient is not None:
            return cfg.dagger_coefficient
        if not cfg.dagger_anneal:
            return cfg.dagger_coefficient_max
        if cfg.dagger_anneal_end <= cfg.dagger_anneal_start:
            return cfg.dagger_coefficient_min
        it = self.current_learning_iteration
        if it <= cfg.dagger_anneal_start:
            return cfg.dagger_coefficient_max
        if it >= cfg.dagger_anneal_end:
            return cfg.dagger_coefficient_min
        ratio = (it - cfg.dagger_anneal_start) / (cfg.dagger_anneal_end - cfg.dagger_anneal_start)
        return cfg.dagger_coefficient_max + (cfg.dagger_coefficient_min - cfg.dagger_coefficient_max) * ratio
    ...
    def learn(self):
        ...
        if self.config.distill.enabled and self.config.distill.dagger_anneal:
            dagger_coeff = self._get_dagger_coefficient()
            if dagger_coeff <= self.config.distill.dagger_coefficient_min + 1e-8:
                logger.warning(
                    "DAgger annealing is enabled but current coefficient is already at minimum "
                    f"({dagger_coeff:.4f}) at iteration {self.current_learning_iteration}. "
                    "If this run resumed from a checkpoint, check dagger_anneal_start/end."
                )
    ...
    def _rollout_step(self, obs_dict):
        with torch.inference_mode():
            for _ in range(self.config.num_steps_per_env):
                ...
                gt_actions = None
                if self.config.distill.enabled:
                    teacher_obs = torch.cat([obs_dict[k] for k in self.config.distill.teacher_obs_keys], dim=1)
                    gt_actions = self.teacher_actor.act_inference({"actor_obs": teacher_obs}).detach()
    ...
    def _update_algo_step(self, minibatch: Minibatch, loss_dict: dict[str, float]):
        ...
        total_loss = ppo_loss
        if self.config.distill.enabled:
            gt_actions = minibatch["gt_actions"]
            pred_actions = self.actor.act_inference({"actor_obs": minibatch["actor_obs"]})
            kin_loss = F.mse_loss(pred_actions, gt_actions)
            dagger_coeff = self._get_dagger_coefficient()
            kin_loss = kin_loss * dagger_coeff

            if self.config.distill.dagger_anneal:
                total_loss = total_loss * (1.0 - dagger_coeff)
            if self.config.distill.dagger_only:
                total_loss = total_loss * 0.0
            total_loss = total_loss + kin_loss

            loss_dict.setdefault("Kin", 0.0)
            loss_dict["Kin"] += kin_loss.item()
