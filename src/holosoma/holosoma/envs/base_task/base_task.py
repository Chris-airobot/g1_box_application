...
class BaseTask:
    ...
    def _check_termination(self):
        ...
        term_results = getattr(self.termination_manager, "last_term_results", {})
        if term_results:
            reset_mask = reset_flags | timeout_flags
            for term_name, result in term_results.items():
                triggered = result & reset_mask
                self.log_dict[f"termination/{term_name}"] = triggered.float().mean()

            if self.is_evaluating and reset_mask.any():
                reset_envs = reset_mask.nonzero(as_tuple=False).flatten().tolist()
                reasons = [name for name, result in term_results.items() if (result & reset_mask).any()]
                parent_terms = {name.split("/", 1)[0] for name in reasons if "/" in name}
                if parent_terms:
                    reasons = [name for name in reasons if name not in parent_terms]
                if reasons:
                    logger.info(f"Reset envs {reset_envs} due to: {', '.join(reasons)}")
