...
class WholeBodyTrackingManager(BaseTask):
    ...
    def _draw_debug_vis_isaacsim(self):
        ...
        if not hasattr(motion_command, "visualization_markers"):
            return
        # Align reference keypoints to robot Z for visualization-only comparison.
        ref_pos_w = motion_command.ref_pos_w
        robot_ref_pos_w = motion_command.robot_ref_pos_w
        z_offset = ref_pos_w[:, 2] - robot_ref_pos_w[:, 2]
        motion_robot_pos_xyz = ref_pos_w.clone()
        motion_robot_pos_xyz[:, 2] = robot_ref_pos_w[:, 2]
        body_pos_vis = motion_command.body_pos_relative_w.clone()
        body_pos_vis[..., 2] -= z_offset[:, None]
        # torso link
        ...
        for body_idx, body_names in enumerate(motion_command.motion_cfg.body_names_to_track):
            motion_robot_body_pos_xyz = body_pos_vis[0, body_idx].clone()
            motion_command.visualization_markers[f"motion_{body_names}"].visualize(
                motion_robot_body_pos_xyz.unsqueeze(0)
            )
            real_robot_body_pos_xyz = motion_command.robot_body_pos_w[0, body_idx].clone()
            motion_command.visualization_markers[f"real_{body_names}"].visualize(
                real_robot_body_pos_xyz.unsqueeze(0)
            )
    ...
    def _draw_debug_vis_isaacgym(self):
        ...
        robot_ref_pos_w = motion_command.robot_ref_pos_w
        z_offset = ref_pos_w[:, 2] - robot_ref_pos_w[:, 2]
        ref_pos_w_vis = ref_pos_w.clone()
        ref_pos_w_vis[:, 2] = robot_ref_pos_w[:, 2]
        body_pos_vis = motion_command.body_pos_relative_w.clone()
        body_pos_vis[..., 2] -= z_offset[:, None]
        ...
        for env_id in range(self.num_envs):
            for body_idx in range(n_bodies):
                ...
                self.simulator.draw_sphere(
                    body_pos_vis[env_id, body_idx], 0.03, color, env_id, body_idx
                )
            ...
            color = (0.0, 1.0, 0.0)
            self.simulator.draw_sphere(ref_pos_w_vis[env_id], 0.05, color, env_id, n_bodies * 2 + 0)
            color = (0.0, 0.0, 1.0)
            self.simulator.draw_sphere(
    ...
    def _post_compute_observations_callback(self):
        if not self.simulator.debug_viz_enabled:
            return
        if self.viewer:
    ...
    def step_visualize_motion(self, actions):
        motion_command = self.command_manager.get_state("motion_command")
        fps = getattr(motion_command, "motion_fps", motion_command.motion.fps)
        dt = 1.0 / float(fps)
        motion_command.step()
        ...
        if motion_command.motion.has_object:
            ...
            object_ang_vel = motion_command.object_ang_vel_w.clone()
            ...
            object_states[:, 10:13] = object_ang_vel[:]
        ...
        if hasattr(motion_command, "segment_end_steps"):
            return motion_command.time_steps[0].item() >= motion_command.segment_end_steps[0].item() - 2
