...
def _isaacsim_randomize_rigid_body_mass(
    ...
    # IsaacLab v2.x exposes `mdp.randomize_rigid_body_mass` as an EventTerm class
    # (ManagerTermBase), not a plain function. It must be constructed with an
    # EventTermCfg and then called with (env, env_ids, ...).
    term = mdp.randomize_rigid_body_mass(
        EventTermCfg(
