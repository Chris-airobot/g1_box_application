...
def robot_body_lin_vel_b(env: WholeBodyTrackingManager) -> torch.Tensor:
    motion_command = _get_motion_command_and_assert_type(env)
    vel_b = quat_rotate_inverse_batched(motion_command.robot_ref_quat_w, motion_command.robot_body_lin_vel_w)
    return vel_b.view(env.num_envs, -1)


def robot_body_ang_vel_b(env: WholeBodyTrackingManager) -> torch.Tensor:
    motion_command = _get_motion_command_and_assert_type(env)
    vel_b = quat_rotate_inverse_batched(motion_command.robot_ref_quat_w, motion_command.robot_body_ang_vel_w)
    return vel_b.view(env.num_envs, -1)
...
def obj_target_pos_b(env: WholeBodyTrackingManager) -> torch.Tensor:
    ...
    pos_b, _ = subtract_frame_transforms(
        motion_command.robot_ref_pos_w,
        motion_command.robot_ref_quat_w,
        motion_command.object_target_pos_w,
        unit_quat,
    )
    return pos_b.view(env.num_envs, -1)


def obj_pos_real_b(env: WholeBodyTrackingManager) -> torch.Tensor:
    motion_command = _get_motion_command_and_assert_type(env)
    root_states = env.simulator.robot_root_states
    base_pos_w = root_states[:, :3]
    base_quat_w = root_states[:, 3:7]
    obj_pos_w = motion_command.simulator_object_pos_w
    return quat_rotate_inverse(base_quat_w, obj_pos_w - base_pos_w, w_last=True).view(env.num_envs, -1)


def obj_target_pos_real_b(env: WholeBodyTrackingManager) -> torch.Tensor:
    motion_command = _get_motion_command_and_assert_type(env)
    root_states = env.simulator.robot_root_states
    base_pos_w = root_states[:, :3]
    base_quat_w = root_states[:, 3:7]
    target_pos_w = motion_command.object_target_pos_w
    return quat_rotate_inverse(base_quat_w, target_pos_w - base_pos_w, w_last=True).view(env.num_envs, -1)


def obj_bbox_corners_b(env: WholeBodyTrackingManager) -> torch.Tensor:
    motion_command = _get_motion_command_and_assert_type(env)
    corners_local = motion_command.object_bbox_corners_local
    if corners_local.numel() == 0:
