    ...
    def check(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        for term_name, term_cfg in zip(self._term_names, self._term_cfgs):
            if term_name in self._term_instances:
                ...
                result = instance(self.env, **term_cfg.params)
            else:
            ...
            if term_name in self._term_instances:
                subterms = getattr(instance, "last_subterm_results", None)
                if isinstance(subterms, dict):
                    for sub_name, sub_result in subterms.items():
                        if isinstance(sub_result, torch.Tensor):
                            term_results[f"{term_name}/{sub_name}"] = sub_result
