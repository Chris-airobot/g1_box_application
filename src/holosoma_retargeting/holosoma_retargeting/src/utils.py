...
def calculate_scale_factor(task_name, robot_height):
    ...
    package_path = base_dir / "demo_data" / "height_dict.pkl"
    cwd_path = Path("demo_data/height_dict.pkl")
    height_dict_path = package_path if package_path.exists() else cwd_path
    if not height_dict_path.exists():
        raise FileNotFoundError(
            "height_dict.pkl not found. Tried: "
            f"{package_path} and {cwd_path}"
        )
...
def create_box_mesh_and_urdf(
    box_size: np.ndarray | list[float] | tuple[float, float, float],
    object_name: str | None = None,
    models_root: str | Path | None = None,
) -> tuple[str, Path, Path]:
    """Create a box mesh + URDF under models/ and return (name, mesh, urdf) paths.

    box_size is expected to be full extents (x, y, z) in meters.
    """
    size = np.asarray(box_size, dtype=float).reshape(-1)
    if size.size != 3:
        raise ValueError(f"box_size must be 3 values, got shape {np.asarray(box_size).shape}")
    if np.any(size <= 0):
        raise ValueError(f"box_size must be positive, got {size}")

    if object_name is None:
        token = "_".join(_format_size_token(float(v)) for v in size)
        object_name = f"box_{token}"

    models_root_path = _resolve_models_root(models_root)
    object_dir = models_root_path / object_name
...
"""
    ...
    return object_name, mesh_path, urdf_path


def create_robot_object_xml_from_template(
    robot_urdf_file: str | Path,
    object_name: str,
    template_object_name: str = "largebox",
) -> Path:
    """Create a MuJoCo robot+object XML by templating the largebox scene."""
    robot_urdf_path = Path(robot_urdf_file)
    if not robot_urdf_path.is_absolute():
        robot_urdf_path = (Path(__file__).resolve().parents[1] / robot_urdf_path).resolve()

    template_xml = robot_urdf_path.with_name(f"{robot_urdf_path.stem}_w_{template_object_name}.xml")
    if not template_xml.exists():
        raise FileNotFoundError(f"Template XML not found: {template_xml}")

    output_xml = robot_urdf_path.with_name(f"{robot_urdf_path.stem}_w_{object_name}.xml")
    if output_xml.exists():
        return output_xml

    content = template_xml.read_text()
    content = content.replace(template_object_name, object_name)
