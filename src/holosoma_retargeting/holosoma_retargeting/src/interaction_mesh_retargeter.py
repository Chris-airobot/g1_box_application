...
class InteractionMeshRetargeter:
    ...
    def retarget_motion(
        ...
        with tqdm(range(num_frames)) as pbar:
            for i in pbar:
                ...
                # Contact-aware hand weighting from the SOURCE demonstration.
                #
                # human_mapped_joints_in_object and object_points_local_demo are
                # both expressed in the demo object's local frame.
                hand_contact_weights = {}

                if self.object_name != "ground":
                    mapped_keys = list(self.laplacian_match_links.keys())

                    # For our centered box meshes this recovers the scaled
                    # half-extents of the demonstration object.
                    box_half = np.max(np.abs(object_points_local_demo), axis=0)

                    contact_on = 0.01   # <= 1 cm: full contact weight
                    contact_off = 0.02  # >= 2 cm: ordinary weight
                    max_weight = 10.0

                    for hand_key in ("LeftHandMiddle3", "RightHandMiddle3"):
                        if hand_key not in mapped_keys:
                            continue

                        hand_idx = mapped_keys.index(hand_key)
                        p_hand = human_mapped_joints_in_object[hand_idx]

                        # Euclidean distance from point to axis-aligned box
                        # surface in object-local coordinates.
                        outside = np.maximum(np.abs(p_hand) - box_half, 0.0)
                        dist = float(np.linalg.norm(outside))

                        if dist <= contact_on:
                            weight = max_weight
                        elif dist >= contact_off:
                            weight = 1.0
                        else:
                            alpha = (contact_off - dist) / (contact_off - contact_on)
                            weight = 1.0 + alpha * (max_weight - 1.0)

                        hand_contact_weights[hand_key] = weight
                ...
                q, cost = self.iterate(
                    ...
                    hand_contact_weights=hand_contact_weights,
                    init_t=i == 0,
    ...
    def solve_single_iteration(
        ...
        hand_contact_weights: dict[str, float] | None = None,
        verbose=False,
        init_t=False,
    ):
        ...
        # Contact-aware hand weighting:
        # normal tracking while approaching, strong interaction preservation
        # only when the source HiPHI fingertip is actually near the object.
        if hand_contact_weights is not None:
            for i, key in enumerate(robot_link_keys):
                if key in hand_contact_weights:
                    w_v[i] *= float(hand_contact_weights[key])
    ...
    def _calc_manipulator_jacobians(
        ...
        for name, link_name in links.items():
            ...
            if body_id == -1:
                raise ValueError(
                    f"Mapped body '{link_name}' for demo joint '{name}' "
                    "does not exist in the loaded MuJoCo model"
                )
