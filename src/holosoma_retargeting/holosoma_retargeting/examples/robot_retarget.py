...
from holosoma_retargeting.src.utils import (  # noqa: E402
    ...
    create_box_mesh_and_urdf,
    create_new_scene_xml_file,
    create_robot_object_xml_from_template,
...
def create_task_constants(
    ...
    task_constants.ROBOT_URDF_FILE = _resolve_asset_path(getattr(task_constants, "ROBOT_URDF_FILE", None))
    task_constants.OBJECT_URDF_FILE = _resolve_asset_path(getattr(task_constants, "OBJECT_URDF_FILE", None))
    task_constants.OBJECT_MESH_FILE = _resolve_asset_path(getattr(task_constants, "OBJECT_MESH_FILE", None))
    task_constants.OBJECT_URDF_TEMPLATE = _resolve_asset_path(getattr(task_constants, "OBJECT_URDF_TEMPLATE", None))
    ...
    if task_type == "robot_only":
        ...
        if data_format == "stickman":
            ...
            if object_poses.shape[0] != human_joints.shape[0]:
                raise ValueError(
                    f"object_poses frame count {object_poses.shape[0]} does not match "
                    f"human_joints frame count {human_joints.shape[0]}"
                )
            human_height = float(np.array(human_data["height"]).reshape(-1)[0])
            smpl_scale = constants.ROBOT_HEIGHT / human_height
            box_size_key = None
            if "box_size" in human_data:
                box_size_key = "box_size"
            elif "boxsize" in human_data:
                box_size_key = "boxsize"
            if box_size_key is not None:
                raw_box_size = np.array(human_data[box_size_key])
                if raw_box_size.ndim == 2 and raw_box_size.shape[0] > 0:
                    box_size = raw_box_size[0]
                else:
                    box_size = raw_box_size.reshape(-1)
                if box_size.size != 3:
                    raise ValueError(f"box_size must have 3 values, got shape {raw_box_size.shape}")
                object_name, mesh_path, urdf_path = create_box_mesh_and_urdf(box_size)
                create_robot_object_xml_from_template(constants.ROBOT_URDF_FILE, object_name)
                constants.OBJECT_NAME = object_name
                constants.OBJECT_MESH_FILE = str(mesh_path)
                constants.OBJECT_URDF_FILE = str(urdf_path)
                constants.OBJECT_URDF_TEMPLATE = None
        else:
            pt_path = data_path / f"{task_name}.pt"
            if not pt_path.exists():
                raise FileNotFoundError(f"InterMimic data file not found: {pt_path}")
...
def main(cfg: RetargetingConfig) -> None:
    ...
    if task_type == "object_interaction":
        ...
        for key in foot_sticking_sequences[0]:
            foot_sticking_sequences[0][key] = False
