...
RESULT_DIR = Path("/fred/oz430/tliu/data/HiPHI/retarget_recovered13")
RETARGET = (
    ROOT
...
def evaluate(task, source_path, result_path):
    ...
    # Pickup = first time object moves >2 cm upward.
    z = obj[:, 6]

    pickup_candidates = np.where(
        z > z[0] + PICKUP_Z_THRESH
    )[0]

    if len(pickup_candidates) == 0:
        pickup = int(np.argmin(source_L + source_R))
    else:
        pickup = int(pickup_candidates[0])
